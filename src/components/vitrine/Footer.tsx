...
const colonnes = [
  ...
  },
  {
    titre: "Ressources",
    liens: [
      { href: "/#checklist", label: "Checklist contrôle CNAPS" },
    ...
    <footer className="bg-encre text-papier">
      <Conteneur large className="py-16 md:py-20">
        <div className="grid gap-12 md:grid-cols-[1.3fr_repeat(3,1fr)] md:gap-8">
          <div className="space-y-5">
            <Link href="/" className="flex items-baseline gap-2">
              <span className="font-display text-[28px] leading-none">ANM</span>
              <span className="font-mono text-[11px] uppercase tracking-[0.28em] text-brume">Consulting</span>
            </Link>
            <p className="max-w-xs text-[14px] leading-relaxed text-brume-2">
              Audit, préparation aux contrôles et formation pour les entreprises de sécurité privée. Vingt ans de
              direction dans le secteur.
            </p>
            <p className="text-[13px] leading-relaxed text-brume">
          ...
          {colonnes.map((c) => (
            <div key={c.titre} className="space-y-4">
              ...
              <ul className="space-y-2.5">
                {c.liens.map((l) => (
                  <li key={l.href}>
                    <Link href={l.href} className="souligne pb-0.5 text-[14px] text-brume-2 hover:text-papier">
                      {l.label}
                    </Link>
                  </li>
                ))}
        ...
        <div className="mt-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <p className="font-mono text-[11px] tracking-wide text-brume">
            © {new Date().getFullYear()} ANM Consulting · {EDITEUR.denomination} · SIREN {EDITEUR.siren}
          </p>
          <ul className="flex flex-wrap gap-x-6 gap-y-2">
            {legal.map((l) => (
              <li key={l.href}>
                <Link href={l.href} className="souligne pb-0.5 text-[12px] text-brume hover:text-papier">
                  {l.label}
                </Link>
              </li>
            ))}
