...
export const lireDonneesST = async (missionId: string): Promise<DonneesST> => {
  ...
  const [p, v, b, s, a, f, pa, sm] = await Promise.all([
    ...
    supabase.from("st_ventes").select("id, mois, client, bon_commande, heures_commandees, heures_facturees, montant_ht, note").eq("mission_id", missionId).order("mois").order("created_at"),
    supabase.from("st_paie").select("mois, effectif, heures_payees, note").eq("mission_id", missionId).order("mois"),
  ...
  return {
    parametres: param,
    ventes: nombres(v.data as Record<string, unknown>[] | null, ["heures_commandees", "heures_facturees", "montant_ht"]) as unknown as Vente[],
    paie: nombres(b.data as Record<string, unknown>[] | null, ["effectif", "heures_payees"]) as unknown as Paie[],
