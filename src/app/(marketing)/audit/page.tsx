  ...
  return (
    <>
      ...
      <section className="relative overflow-hidden">
        <div className="pointer-events-none absolute -left-40 top-0 size-[560px] rounded-full bg-menthe/40 blur-3xl" aria-hidden />
        <Conteneur large className="relative grid gap-12 pb-20 pt-12 md:grid-cols-[1.1fr_0.9fr] md:items-center md:pb-28 md:pt-20">
          <div className="space-y-7">
            ...
            <Reveal delay={0.05}>
              <h1 className="font-display text-[2.8rem] leading-[1.02] text-encre md:text-[4.2rem]">
                Une photographie factuelle, <em className="text-vert">un plan daté.</em>
              </h1>
            </Reveal>
            <Reveal delay={0.1}>
              <p className="max-w-xl text-[17px] leading-relaxed text-encre-2">
      ...
      <section>
        <Conteneur className="py-24 md:py-32">
          <div className="grid gap-12 lg:grid-cols-[0.9fr_1.1fr] lg:gap-20">
            ...
            />
            <Cascade className="grid gap-px overflow-hidden rounded-[5px] border border-filet bg-filet sm:grid-cols-3" pas={0.1}>
              {ECHANTILLON.map((e, i) => (
                <Element key={e.titre} className="space-y-3 bg-papier p-6">
                  <span className="font-mono text-[11px] text-gris">0{i + 1}</span>
                  <h3 className="font-display text-[1.35rem] leading-snug text-encre">{e.titre}</h3>
                  <p className="text-[14px] leading-relaxed text-encre-2">{e.texte}</p>
                </Element>
              ))}
            </Cascade>
          </div>
              ...
              <div className="grid gap-px overflow-hidden rounded-[5px] border border-filet bg-filet sm:grid-cols-2">
                {AXES_RAPPORT.map((a) => (
                  <div key={a.nature} className={`space-y-2 p-6 md:p-8 ${a.nature === "risque_controle" ? "bg-critique-l/50" : "bg-menthe-2"}`}>
                    <h3 className={`font-display text-[1.5rem] leading-tight ${a.nature === "risque_controle" ? "text-critique" : "text-vert"}`}>{a.titre}</h3>
                    <p className="text-[14px] leading-relaxed text-encre-2">{a.definition}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <Cascade className="rounded-[5px] border border-encre bg-papier" pas={0.08}>
              {RESTITUTION.map((r, i) => (
                <Element key={r.titre} className="grid grid-cols-[3rem_1fr] gap-4 border-b border-filet px-6 py-5 last:border-0" y={10}>
                  <span className="font-mono text-[12px] text-gris">0{i + 1}</span>
                  <div className="space-y-1">
                    <h3 className="font-display text-[1.3rem] leading-snug text-encre">{r.titre}</h3>
                    <p className="text-[14px] leading-relaxed text-encre-2">{r.texte}</p>
                  </div>
                </Element>
              ))}
            </Cascade>
          ...
          <div id="estimateur" className="mt-20 scroll-mt-28 grid gap-8 md:grid-cols-[0.8fr_1.2fr] md:items-start">
            <Reveal>
              <div className="space-y-4 md:sticky md:top-28">
                ...
                <h3 className="font-display text-[2rem] leading-[1.08] text-encre md:text-[2.4rem]">Estimez votre mission.</h3>
