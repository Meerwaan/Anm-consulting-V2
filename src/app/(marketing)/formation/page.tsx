...
import { CtaFinal, Reperes } from "@/components/vitrine/Sections";
...
export const metadata: Metadata = {
  ...
  description: "26 modules, 9 parcours : CNAPS, social et URSSAF, temps de travail, Inspection du travail, sous-traitance, fiscal. Vidéos, fiches, quiz, outils interactifs. Premier module gratuit.",
...
const FORMATS = [
  {
    nom: "Vidéo",
    texte: "Quinze à trente minutes, une seule idée par module, des exemples de terrain.",
  },
  {
    nom: "Fiche",
    texte: "La règle, la référence officielle datée, la formulation de constat recommandée.",
  },
  {
    nom: "Checklist",
    texte: "À télécharger, à imprimer, à cocher avant un contrôle ou une embauche.",
  },
  {
    nom: "Outil interactif",
    texte: "Auto-diagnostic, rapprochement heures, suivi des cartes : vous saisissez, l’outil montre l’écart.",
  },
...
export default function FormationPage() {
  return (
    <>
      <section className="relative overflow-hidden">
        <Conteneur className="relative grid gap-14 pb-20 pt-12 md:grid-cols-[1.15fr_0.85fr] md:items-end md:gap-20 md:pb-28 md:pt-20">
          <div className="space-y-8">
            ...
            <Reveal delay={0.15}>
              <div className="rounded-[5px] border border-majeur/40 bg-sable/60 px-5 py-4 text-corps text-encre">
                <span className="etiquette mr-3 !text-majeur">Ouverture</span>
                Le catalogue est en préparation. Les contenus, formats et tarifs ci-dessous sont indicatifs et seront précisés avec l’ouverture des premiers parcours. [Date d’ouverture à définir]
              </div>
            </Reveal>
            <Reveal delay={0.2}>
          ...
          <Reveal delay={0.2} y={32}>
            <Reperes
              titre="Le catalogue en quatre chiffres"
              lignes={[
                {
                  cle: "Parcours",
                  valeur: String(PARCOURS.length),
                  href: "#parcours",
                },
                { cle: "Modules courts", valeur: String(NB_MODULES) },
                {
                  cle: "Formats",
                  valeur: `${FORMATS.length}, du quiz à l’outil`,
                },
                { cle: "Premier module", valeur: "Gratuit" },
              ]}
            />
          </Reveal>
        </Conteneur>
      ...
      <section id="parcours" className="scroll-mt-20 bg-papier">
        <Conteneur className="py-24 md:py-32">
          <SectionHead
            ...
            titre={
              <>
                Neuf parcours, <em className="text-vert">un par sujet de contrôle.</em>
              </>
            }
            aligne="deux"
              ...
              <Reveal key={p.id}>
                <div className="grid gap-6 lg:grid-cols-[0.75fr_1.25fr] lg:gap-14">
                  <div className="space-y-3 lg:sticky lg:top-28 lg:self-start">
                    <span className="font-mono text-etiquette text-gris">{String(pi + 1).padStart(2, "0")}</span>
                    <h3 className="font-display text-t3 text-encre">{p.nom}</h3>
                    <p className="text-corps text-encre-2">{p.accroche}</p>
                    <p className="font-mono text-etiquette uppercase tracking-[0.14em] text-gris">
                    ...
                    <ol>
                      {p.modules.map((m) => (
                        <li key={m.code} className="grid gap-2 border-b border-filet py-4 md:grid-cols-[5.5rem_1fr_auto] md:items-baseline md:gap-6">
                          <span className="font-mono text-etiquette text-gris">{m.code}</span>
                          <div className="space-y-1.5">
                            <p className="font-display text-t4 text-encre">{m.titre}</p>
                          ...
                          <span className={`inline-flex w-fit rounded-full px-2.5 py-1 text-etiquette font-medium ${COULEUR_OFFRE[m.offre]}`}>{LIBELLE_OFFRE[m.offre]}</span>
                        </li>
                      ))}
                    </ol>
                  </div>
      ...
      <section>
        <Conteneur className="py-24 md:py-32">
          <SectionHead
            ...
            titre={
              <>
                Court, concret, <em className="text-vert">réutilisable le lendemain.</em>
              </>
            }
            aligne="deux"
            sous="Pas de cours magistral. Chaque module se termine par quelque chose qu’on peut faire dans l’entreprise."
          />
          <Cascade className="mt-14 grid gap-px overflow-hidden rounded-[5px] border border-filet bg-filet sm:grid-cols-2 lg:grid-cols-5" pas={0.08}>
            {FORMATS.map((f) => (
              <Element key={f.nom} className="space-y-3 bg-papier p-6">
                <h3 className="font-display text-t4 text-encre">{f.nom}</h3>
                <p className="text-meta text-encre-2">{f.texte}</p>
              </Element>
            ))}
          </Cascade>
        </Conteneur>
        ...
        <Conteneur className="py-24 md:py-32">
          <div className="grid gap-12 md:grid-cols-[1.2fr_0.8fr] md:items-end md:gap-20">
            <div className="space-y-6">
              ...
              <Reveal delay={0.1}>
                <p className="font-mono text-note text-brume">{PROGRAMME_30_JOURS.format}</p>
              </Reveal>
            </div>
            <Reveal delay={0.15}>
              <div className="space-y-4 rounded-[5px] border border-nuit p-6 md:p-8">
                ...
                <ul className="space-y-3 text-corps text-brume-2">
                  <li className="flex justify-between gap-4 border-b border-nuit pb-3">
                    <span>Module gratuit</span>
                    <span className="font-mono text-note text-papier">0 €</span>
                  </li>
                  <li className="flex justify-between gap-4 border-b border-nuit pb-3">
                    <span>Module à l’unité</span>
                    <span className="font-mono text-note text-papier">[à définir]</span>
                  </li>
                  <li className="flex justify-between gap-4">
                    <span>Inclus dans l’abonnement conformité</span>
                    <span className="font-mono text-note text-papier">dès 390 € HT / mois</span>
                  </li>
                </ul>
      ...
      <CtaFinal
        ...
        texte="Session intra-entreprise, sur vos plannings et vos bulletins : on part de vos écarts, pas d’un cas d’école."
        cta="En parler"
        href="/contact?situation=formation"
      />
    </>
  );
}
